...
#
# Source: modems/technicolor/tc4400/parser.py
...
class TechnicolorTC4400Parser(ModemParser):
    ...
    def parse_resources(self, resources: dict[str, Any]) -> dict:
        ...
        # Get system info from cmswinfo.html if available
        system_info = {}
        info_soup = resources.get("/cmswinfo.html")
        if info_soup and isinstance(info_soup, BeautifulSoup):
            system_info = self._parse_system_info(info_soup)
        else:
            system_info = self._parse_system_info(soup)

        # Parse channels with system info for restart detection
        downstream = self._parse_downstream(soup, system_info)
        upstream = self._parse_upstream(soup, system_info)
