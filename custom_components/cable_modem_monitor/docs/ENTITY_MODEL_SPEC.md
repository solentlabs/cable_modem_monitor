...
|--------|-----------------|-------|--------------|-------------|------|------------|
| Status | `_status` | enum¹ | — | — | — | `connection_status`, `health_status`, `docsis_status`, `diagnosis` |
| Modem Info | `_info` | detected model | — | — | — | `manufacturer`, `release_date`, `docsis_version`, `fixtures_url`, `parser_verified` |
...
| DS Channel Count | `_downstream_channel_count` | int | — | MEASUREMENT | — | — |
| US Channel Count | `_upstream_channel_count` | int | — | MEASUREMENT | — | — |
| Total Corrected | `_total_corrected` | int | — | TOTAL_INCREASING | — | — |
| Total Uncorrected | `_total_uncorrected` | int | — | TOTAL_INCREASING | — | — |

¹ Status values: Operational, Degraded, Partial Lock, Not Locked,
No Signal, Parser Error, Unresponsive, ICMP Blocked.
...
All four buttons are always created. Restart Modem is marked
unavailable when the modem doesn't support restart (checked once at
setup from modem.yaml `actions.restart`).
...
See [FIELD_REGISTRY.md](../../../packages/cable_modem_monitor_catalog/docs/FIELD_REGISTRY.md)
...
Core owns the data model. HA owns the presentation.
...
The unique_id format is unchanged from v3.13. No entity migration
is needed for the v3.14 upgrade.
