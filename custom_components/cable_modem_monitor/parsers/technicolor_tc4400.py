...
class TechnicolorTC4400Parser(ModemParser):
    ...
    def can_parse(cls, soup: BeautifulSoup, url: str, html: str) -> bool:
        ...
        # Check URL for TC4400 specific endpoint
        if "cmconnectionstatus.html" in url.lower():
            return True

        # Check for Technicolor-specific markers in HTML
        # Note: This parser needs real HTML samples to improve detection
    ...
    def parse_downstream(self, soup: BeautifulSoup) -> list[dict]:
        ...
        try:
            # TODO: Implement TC4400-specific parsing when HTML samples available
            # For now, try generic table parsing similar to Motorola
            ...
            for table in tables:
                ...
                # Look for downstream channel indicators
                header_row = rows[0]
                ...
                if any(keyword in header_text for keyword in ['downstream', 'channel', 'frequency', 'power', 'snr']):
                    ...
                    for row in rows[1:]:
                        ...
                        if len(cols) >= 4:  # At least channel, freq, power, snr
                            try:
                                channel_data = {
        ...
        try:
            ...
            for table in tables:
                ...
                if any(keyword in header_text for keyword in ['upstream', 'channel', 'frequency', 'power']):
                    ...
                    for row in rows[1:]:
                        cols = row.find_all(['td', 'th'])
                        if len(cols) >= 3:  # At least channel, freq, power
                            try:
                                channel_data = {
                                    "channel_id": str(self._extract_number(cols[0].text) or len(channels) + 1),
                                    "frequency": self._parse_frequency(cols[1].text),
    ...
    def _parse_frequency(self, text: str) -> int | None:
        ...
        try:
            ...
            # Check if it's in MHz (TC4400 might use MHz)
