...
class ArrisCM3500BParser(ModemParser):
    ...
    verification_source = "https://github.com/solentlabs/cable_modem_monitor/issues/73"

    # Device metadata
    release_date = None  # Unknown
    docsis_version = "3.1"
    fixtures_path = "tests/parsers/arris/fixtures/cm3500b"

    priority = 100

    # CM3500B uses form-based authentication
    auth_config = FormAuthConfig(
        strategy=AuthStrategyType.FORM_PLAIN,
        login_url="/cgi-bin/login_cgi",
        username_field="username",
        password_field="password",
        success_indicator="status_cgi",
    )

    url_patterns = [
        {"path": "/cgi-bin/status_cgi", "auth_method": "form", "auth_required": True},
        {"path": "/cgi-bin/vers_cgi", "auth_method": "form", "auth_required": True},
    ]
    ...
    def can_parse(cls, soup: BeautifulSoup, url: str, html: str) -> bool:
        ...
        model_td = soup.find("td", string="Hardware Model")
        if model_td:
    ...
    def _parse_downstream_qam(self, soup: BeautifulSoup) -> list[dict]:
        ...
        header = soup.find("h4", string=re.compile(r"Downstream QAM", re.IGNORECASE))
        if not header:
    ...
    def _parse_downstream_ofdm(self, soup: BeautifulSoup) -> list[dict]:
        ...
        header = soup.find("h4", string=re.compile(r"Downstream OFDM", re.IGNORECASE))
        if not header:
    ...
    def _parse_upstream_qam(self, soup: BeautifulSoup) -> list[dict]:
        ...
        header = soup.find("h4", string=re.compile(r"Upstream QAM", re.IGNORECASE))
        if not header:
    ...
    def _parse_upstream_ofdm(self, soup: BeautifulSoup) -> list[dict]:
        ...
        header = soup.find("h4", string=re.compile(r"Upstream OFDM", re.IGNORECASE))
        if not header:
    ...
    def _parse_system_info(self, soup: BeautifulSoup) -> dict:
        ...
        uptime_td = soup.find("td", string=re.compile(r"System Uptime", re.IGNORECASE))
        if uptime_td:
        ...
        # Parse CM status
        status_td = soup.find("td", string=re.compile(r"CM Status", re.IGNORECASE))
        if status_td:
        ...
        # Parse current time
        time_td = soup.find("td", string=re.compile(r"Time and Date", re.IGNORECASE))
        if time_td:
