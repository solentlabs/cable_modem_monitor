...
class MotorolaMBParser(ModemParser):
    ...
    def can_parse(cls, soup: BeautifulSoup, url: str, html: str) -> bool:
        ...
        # Check for Motorola-specific title
        title = soup.find("title")
        if title and "Motorola Cable Modem" in title.text:
            return True

        # Check for Motorola-specific CSS class
        if soup.find("td", class_="moto-param-header-s"):
            return True

        # Check for Motorola Connection page URL
    ...
    def parse_upstream(self, soup: BeautifulSoup) -> list[dict]:
        ...
        try:
            ...
            for table in tables:
                ...
                # Find the header row for upstream channels
                header_row = None
                headers = []
                for row in all_rows:
                    potential_headers = [
                    ...
                    # Upstream tables have "Channel Type" header
                ...
                if header_row and headers:
                    ...
                    for row in data_rows:
                        cols = row.find_all("td")
                        # Upstream table columns: Channel, Lock Status, Channel Type, Channel ID, Symb Rate, Freq (MHz), Pwr (dBmV)
                        if len(cols) >= 7:
                            try:
                                # Extract frequency from column 5 (0-indexed)
                                ...
                                # Extract channel number from column 0
                                channel_id = self._extract_number(cols[0].text)
                                if channel_id is None:
                                    _LOGGER.warning(
                                ...
                                # Skip channels that are not locked (inactive channels)
                                ...
                                channel_data = {
                                    "channel_id": str(channel_id),
                                    "frequency": freq_hz,
                                    "power": self._extract_float(cols[6].text),  # Power is in column 6
                                    "modulation": lock_status,
                                }

                                # Skip if all critical values are None
                                if all(
                                    v is None
                                    for k, v in channel_data.items()
    ...
    def parse_system_info(self, soup: BeautifulSoup) -> dict:
        ...
        try:
            ...
            # Software version and channel counts require MotoHome.asp
            # which needs to be fetched separately by the scraper
            # For now, just return uptime from connection page
    ...
    def _parse_system_uptime(self, soup: BeautifulSoup) -> str:
        """Parse system uptime from MotoConnection.asp page."""
        try:
            # Look for "System Up Time" label
            rows = soup.find_all("tr")

            for row in rows:
                # Check all cells in the row for "System Up Time" text
                cells = row.find_all("td")
                for i, cell in enumerate(cells):
                    if "System Up Time" in cell.text:
                        # Value is typically in the next cell or cell with class
