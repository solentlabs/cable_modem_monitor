...
async def async_get_config_entry_diagnostics(
    ...
    ***REMOVED*** Security: Sanitize exception messages to avoid leaking sensitive information
    if coordinator.last_exception:
        ...
        ***REMOVED*** Sanitize exception message - remove potential credentials, paths, IPs
        import re
        ***REMOVED*** Remove anything that looks like credentials (password=, user=, etc.)
        exception_msg = re.sub(r'(password|passwd|pwd|token|key|secret|auth)[\s]*[=:]\s*[^\s,}]+',
                               r'\1=***REDACTED***', exception_msg, flags=re.IGNORECASE)
        ***REMOVED*** Remove file paths
        exception_msg = re.sub(r'/[^\s,}]+', '/***PATH***', exception_msg)
        ***REMOVED*** Remove IP addresses (basic pattern)
