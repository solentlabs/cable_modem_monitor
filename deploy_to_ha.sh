#!/bin/bash
# Deploy Cable Modem Monitor to Home Assistant
# Usage: ./deploy_to_ha.sh [ha_host] [ha_user] [ha_port]
...
if ! ssh -p "${HA_PORT}" -o ConnectTimeout=5 "${HA_USER}@${HA_HOST}" "echo 'Connection successful'"; then
...
ssh -p "${HA_PORT}" "${HA_USER}@${HA_HOST}" "if [ -d '${HA_CONFIG_PATH}' ]; then cp -r '${HA_CONFIG_PATH}' '${HA_CONFIG_PATH}.backup-$(date +%Y%m%d-%H%M%S)'; echo 'Backup created'; else echo 'No existing installation found'; fi"

echo ""
echo "Creating directory structure..."
ssh -p "${HA_PORT}" "${HA_USER}@${HA_HOST}" "mkdir -p '${HA_CONFIG_PATH}'"

echo ""
echo "Copying integration files..."
scp -P "${HA_PORT}" -r custom_components/cable_modem_monitor/* "${HA_USER}@${HA_HOST}:${HA_CONFIG_PATH}/"
