***REMOVED***!/bin/bash
***REMOVED*** Cable Modem Monitor - Automated Development Environment Setup
***REMOVED*** This script sets up everything you need to start developing
...
***REMOVED*** Test that the command actually works, not just that it exists
PYTHON_CMD=""
...
***REMOVED*** Validate version is numeric
...
***REMOVED*** Step 2: Check for venv module
...
***REMOVED*** Step 3: Clean up dual venv situation
if [ -d "venv" ]; then
...
***REMOVED*** On Windows, pip upgrade can fail if pip is running, so make it non-fatal
$PIP_CMD install --upgrade pip --quiet || print_warning "pip upgrade skipped (will work next run)"
print_success "pip ready"
echo ""

***REMOVED*** Step 6: Install dependencies
***REMOVED*** Use requirements-dev.txt to ensure consistency with CI
...
if [ -f "requirements-dev.txt" ]; then
    ...
    ***REMOVED*** Fallback to manual installation if requirements-dev.txt doesn't exist
...
***REMOVED*** Step 7: Install pre-commit hooks (optional)
...
***REMOVED*** Step 8: Check Docker (optional)
...
***REMOVED*** Step 9: Check VS Code (optional)
...
if command -v code &> /dev/null; then
    ...
    ***REMOVED*** Check for required extensions
    ...
    if [ ${***REMOVED***MISSING_EXTENSIONS[@]} -gt 0 ]; then
...
***REMOVED*** Step 10: Run a quick test
...
***REMOVED*** Step 11: Create .python-version file
...
echo "     make test"
echo "     ***REMOVED*** or: .venv/bin/pytest tests/"
echo ""
...
echo "     ***REMOVED*** Then open http://localhost:8123"
echo ""
echo "  ${CYAN}4. Open in VS Code:${NC}"
echo "     code ."
echo "     ***REMOVED*** Press F1 → 'Dev Containers: Reopen in Container'"
echo ""
