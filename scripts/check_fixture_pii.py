...
# PII patterns to check for (duplicated from html_helper.py to avoid import issues)
PII_PATTERNS = {
...
PII_ALLOWLIST = [
    ...
    "***EMAIL***",
    "***CONFIG_PATH***",
    # Test/dummy MAC addresses used in fixtures
    "aa:bb:cc:dd:ee:ff",
    "dd:ee:ff:aa:bb:cc",
    # All-zeros MAC (common default)
    "00:00:00:00:00:00",
    # RFC 5737 TEST-NET addresses (safe for documentation/examples)
...
# Regex patterns for sanitized IPv6 placeholders (using aaaa:bbbb:cccc:dddd format)
# These use only hex letters (a-f) with no digits to indicate they are sanitized
SANITIZED_IPV6_PATTERN = re.compile(r"^[a-f]{1,4}(:[a-f]{0,4}|::)+:?$", re.IGNORECASE)

# Patterns that look like PII but are actually OK (timestamps, versions)
FALSE_POSITIVE_PATTERNS = [
    # Time stamps (HH:MM:SS or HH:MM)
    r"^\d{2}:\d{2}:\d{2}$",  # 19:10:40
    r"^\d{2}:\d{2}$",  # 19:10
    # Short version numbers that look like IPs (but aren't real IPs)
    r"^\d+\.\d+\.\d+\.\d+$",  # We'll check if it's actually a plausible IP below
]


def is_timestamp(text: str) -> bool:
    """Check if text looks like a timestamp (HH:MM:SS or HH:MM or partial)."""
    # Pure timestamps like 19:10:40 or 02:00 or partial like 19:36:
...
def is_sanitized_ipv6(text: str) -> bool:
    ...
    # e.g., aaaa:bbbb:cccc:dddd:eeee:ffff or abcd::aa:bb:cc:dd
    # Make sure it contains no actual digits (only hex letters a-f)
...
def is_version_number(text: str) -> bool:
    ...
    # Version numbers typically have small first octet and are in context
    # e.g., "5.7.1.5" is clearly a version, "192.168.1.1" is clearly an IP
    if re.match(r"^\d+\.\d+\.\d+\.\d+$", text):
        parts = text.split(".")
        # If first part is small (< 50), it's likely a version
        if int(parts[0]) < 50:
            return True
    return False
...
def check_for_pii(content: str, filename: str = "") -> list[dict]:
    ...
    for pattern_name, pattern in PII_PATTERNS.items():
        ...
        for match in matches:
            ...
            # Skip timestamps that look like IPv6 (e.g., 19:10:40)
            ...
            # Skip version numbers that look like IPs (e.g., 5.7.1.5)
