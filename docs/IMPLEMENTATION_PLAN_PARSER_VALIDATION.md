***REMOVED*** Implementation Plan: Enhanced Parser Diagnostics
...
- **Issue ***REMOVED***4 Example:** User selected "MB8611 (Static)" but modem uses HNAP protocol
- Result: Entities show as unavailable, logs show parsing errors
- User confusion: "Why doesn't it work?"

***REMOVED******REMOVED******REMOVED*** Design Decision

**Enhancement ***REMOVED***1 (Parser Mismatch Validation) was NOT implemented** based on the following rationale:
...
**Enhancement ***REMOVED***2 (Enhanced Diagnostics) WAS implemented** to help troubleshoot issues when users report problems.

---

***REMOVED******REMOVED*** ~~Enhancement ***REMOVED***1: Parser Mismatch Validation~~ (NOT IMPLEMENTED)
...
async def async_step_user(self, user_input=None):
    ...
    if user_input is not None:
        ...
        ***REMOVED*** NEW: Parser mismatch detection
        if user_input.get(CONF_MODEM_CHOICE) and user_input[CONF_MODEM_CHOICE] != "auto":
            ...
            if mismatch_warning:
                ...
                ***REMOVED*** Show form again with warning
                return self.async_show_form(...)

        ***REMOVED*** ... continue with setup ...
```
...
async def _check_parser_mismatch(
    ...
    """
    ***REMOVED*** Quick probe of modem to get initial HTML
    try:
        ...
        return None  ***REMOVED*** Can't probe, allow user to proceed

    ***REMOVED*** Check for known mismatches
    ...
    ***REMOVED*** Add more mismatch checks as needed

    return None  ***REMOVED*** No mismatch detected
```
...
***REMOVED******REMOVED*** Enhancement ***REMOVED***2: Enhanced Diagnostics Output
    ...
    ***REMOVED*** ... existing code ...
    ...
    diagnostics_data = {
        "config_entry": {
            ...
            ***REMOVED*** NEW: Parser detection info
            "parser_detection": {
        ...
        ***REMOVED*** ... existing modem_data, etc. ...
    }
...
```python
***REMOVED*** In modem_scraper.py _detect_parser():
def _detect_parser(...) -> ModemParser | None:
    """Detect parser with history tracking."""
    attempted_parsers: list[str] = []

    ***REMOVED*** ... existing detection code ...

    ***REMOVED*** Store history for diagnostics
    ...
    ***REMOVED*** ... rest of method ...
```

**Include in diagnostics:**

```python
***REMOVED*** In diagnostics.py:
...
***REMOVED******REMOVED******REMOVED*** Test Cases
...
```bash
***REMOVED*** Test 1: Parser mismatch
...
2. **Beta Testing:** Test with users experiencing Issue ***REMOVED***4
...
***REMOVED******REMOVED******REMOVED*** Phase 2 (Post v3.4.0)
...
- [x] Document design decision (Enhancement ***REMOVED***1 not implemented)
...
**Status:** ✅ IMPLEMENTED (Enhancement ***REMOVED***2 only)
