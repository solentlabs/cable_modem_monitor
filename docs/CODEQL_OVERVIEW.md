# CodeQL Security Scanning - Overview
...
## How CodeQL Works in This Project

### 1. **Automatic Scanning (GitHub Actions)**
...
#### **Custom Queries** (specific to this project)
...
### 3. **Query Filters (False Positive Suppression)**
...
├── .github/
│   ├── workflows/
│   │   └── codeql.yml              # GitHub Actions workflow (runs scans)
│   └── codeql/
│       ├── codeql-config.yml       # Filter config (false positive suppression)
│       └── queries/
│           ├── qlpack.yml          # CodeQL package definition
│           ├── cable-modem-security.qls  # Query suite (lists all queries)
...
│
├── cable-modem-monitor-ql/         # Development/testing sandbox
│   ├── qlpack.yml
│   ├── queries/                    # Test new queries here
│   ├── tests/                      # Unit tests for queries
│   └── README.md
│
└── docs/
    ├── CODEQL_OVERVIEW.md          # This file (high-level overview)
    └── CODEQL_TESTING_GUIDE.md     # How to test queries locally
```
...
### For Most Developers (Just Want Secure Code)
...
```bash
# Run local CodeQL tests (fast, validates query syntax)
...
### For Query Developers (Creating New Security Checks)

**Want to add a new security check?**

1. **Create query in dev sandbox:**
   ```bash
   # Work in development directory first
...
2. **Add test cases:**
   ...
   vim cable-modem-monitor-ql/tests/my-new-check/test.py  # Sample bad code
   vim cable-modem-monitor-ql/tests/my-new-check/test.ql  # Import your query
   vim cable-modem-monitor-ql/tests/my-new-check/test.expected  # Expected results
   ```
...
5. **Push and verify:**
   ...
   # Check GitHub Actions logs to verify query runs
   gh run watch
   ```

## Common Questions

### Q: Where do I see CodeQL results?
...
### Q: CodeQL flagged intentional code. What do I do?

**Option 1: Justify in code (preferred)**
```python
# Cable modems use self-signed certificates on private LANs (192.168.x.x)
# MITM risk is acceptable in this controlled environment. See const.py for rationale.
...
### Q: How do I test if my code will pass CodeQL?
...
### Q: What's the difference between `.github/codeql/queries/` and `cable-modem-monitor-ql/`?
