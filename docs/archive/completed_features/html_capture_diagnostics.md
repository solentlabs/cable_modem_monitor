***REMOVED*** HTML Capture for Diagnostics
...
***REMOVED******REMOVED******REMOVED*** **Phase 1: Button Entity (Existing Installations)**
...
***REMOVED******REMOVED******REMOVED*** **Phase 2: Failed Setup Flow (New Installations)**
...
***REMOVED******REMOVED*** Pages to Capture

***REMOVED******REMOVED******REMOVED*** **Phase 1 - Core Pages (7 pages, ~50-150KB)**

```python
pages_to_capture = [
    ***REMOVED*** Universal attempts (no auth)
    "/",
    "/cmSignalData.htm",           ***REMOVED*** ARRIS

    ***REMOVED*** Motorola (most common)
    "/MotoConnection.asp",         ***REMOVED*** Signal data - CRITICAL
    "/MotoHome.asp",               ***REMOVED*** System info
    "/MotoSwInfo.asp",             ***REMOVED*** Software version
...
***REMOVED******REMOVED******REMOVED*** **Phase 2 - Extended Pages (if needed)**

```python
extended_pages = [
    "/Login.html",                 ***REMOVED*** Shows auth method
    "/cmswinfo.html",              ***REMOVED*** TC4400 software
    "/at_a_glance.jst",           ***REMOVED*** XB7 dashboard
    "/HNAP1/",                    ***REMOVED*** MB8611 HNAP
]
```

***REMOVED******REMOVED*** HTML Sanitization

***REMOVED******REMOVED******REMOVED*** **Information Removed**
...
***REMOVED******REMOVED******REMOVED*** **Information Preserved (for debugging)**
...
***REMOVED******REMOVED******REMOVED*** **Sanitization Function**

```python
def _sanitize_html(html: str) -> str:
    """Sanitize HTML by removing personal/network identifiers."""

    ***REMOVED*** MAC addresses (multiple formats)
    html = re.sub(
    ...
    ***REMOVED*** Serial numbers (common patterns)
    html = re.sub(
    ...
    ***REMOVED*** Private IPs (keep 192.168.100.1 for context)
    html = re.sub(
...
***REMOVED******REMOVED*** Output Structure

***REMOVED******REMOVED******REMOVED*** **ZIP File Contents**

```
modem_html_capture_20251108_223045.zip
├── capture_info.json          ***REMOVED*** Metadata
├── MotoConnection.asp.html    ***REMOVED*** Sanitized HTML
├── MotoHome.asp.html          ***REMOVED*** Sanitized HTML
├── MotoSwInfo.asp.html        ***REMOVED*** Sanitized HTML
└── README.txt                 ***REMOVED*** Instructions
```

***REMOVED******REMOVED******REMOVED*** **capture_info.json**
...
***REMOVED******REMOVED*** Implementation Components

***REMOVED******REMOVED******REMOVED*** **1. Button Entity**

```python
***REMOVED*** custom_components/cable_modem_monitor/button.py
...
class CaptureModemHTMLButton(CoordinatorEntity, ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        try:
            ***REMOVED*** Capture HTML from modem
            html_data = await self._capture_html_pages()

            ***REMOVED*** Create ZIP file in memory
            zip_buffer = self._create_zip_file(html_data)

            ***REMOVED*** Trigger browser download
...
***REMOVED******REMOVED******REMOVED*** **2. Config Flow Enhancement**

```python
***REMOVED*** custom_components/cable_modem_monitor/config_flow.py
...
async def async_step_unsupported_modem(
    ...
    if user_input is not None:
        if user_input.get("capture_html"):
            ***REMOVED*** User wants to help - capture HTML
            try:
                html_data = await self._capture_html_pages()
                zip_buffer = self._create_zip_file(html_data)

                ***REMOVED*** Store in context for download
                self.context["html_capture"] = zip_buffer

                ***REMOVED*** Show instructions
            ...
            ***REMOVED*** User doesn't want to help
            return self.async_abort(reason="unsupported_modem")

    ***REMOVED*** Show opt-in form
...
***REMOVED******REMOVED******REMOVED*** **3. HTML Capture Utility**

```python
***REMOVED*** custom_components/cable_modem_monitor/core/html_capture.py
...
***REMOVED******REMOVED*** Size Limits and Performance
...
***REMOVED******REMOVED*** User Documentation Updates

***REMOVED******REMOVED******REMOVED*** **TROUBLESHOOTING.md**

Add section:
```markdown
***REMOVED******REMOVED******REMOVED*** Capturing Modem HTML for Unsupported Modems
...
***REMOVED******REMOVED******REMOVED*** **Bug Report Template**
...
- Issue ***REMOVED***3: Netgear CM600 support (needs HTML)
- Issue ***REMOVED***4: All entities unavailable (MB8611 - needs HTML to debug)
- Issue ***REMOVED***6: MB8611 connection failed (needs HTML)
