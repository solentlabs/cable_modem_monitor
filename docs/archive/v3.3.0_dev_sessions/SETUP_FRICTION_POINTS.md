***REMOVED*** Setup Friction Points & Fixes
...
***REMOVED******REMOVED*** Issues Found During Testing

***REMOVED******REMOVED******REMOVED*** 1. ❌ `scripts/setup.sh` Fails on Windows
...
```bash
***REMOVED*** Check for python3 first, fall back to python
...
***REMOVED******REMOVED******REMOVED*** 2. ❌ `requirements-dev.txt` Dependency Conflicts
...
```txt
pytest>=7.4.3  ***REMOVED*** Remove <8.0 upper bound
```

**Option B:** Pin homeassistant to compatible version
```txt
homeassistant>=2024.11.0,<2025.1.0  ***REMOVED*** Use last compatible version
```

**Option C:** Separate test requirements
```txt
***REMOVED*** requirements-dev.txt (for VSCode, linting, formatting)
pytest>=7.4.3,<9.0
black>=23.11.0,<25.0
ruff>=0.8.2,<1.0
mypy>=1.7.0,<2.0
beautifulsoup4>=4.12.0,<5.0  ***REMOVED*** ADD THIS
lxml>=4.9.0,<5.0

***REMOVED*** tests/requirements.txt (for full test suite with HA)
...
***REMOVED******REMOVED******REMOVED*** 3. ✅ VSCode Settings Path Issues (FIXED)
...
***REMOVED******REMOVED*** Proposed Changes to Repository

***REMOVED******REMOVED******REMOVED*** File: `scripts/setup.sh`

**Change:** Make Python detection cross-platform

```bash
***REMOVED*** Step 1: Check Python version
print_step "Checking Python version..."

***REMOVED*** Check for python3 first (Linux/macOS), fall back to python (Windows)
if command -v python3 &> /dev/null; then
    PYTHON_CMD="python3"
elif command -v python &> /dev/null; then
    PYTHON_CMD="python"
...
***REMOVED*** ... rest of script uses $PYTHON_CMD instead of python3
```

***REMOVED******REMOVED******REMOVED*** File: `requirements-dev.txt`

**Change:** Add missing dependency, loosen pytest constraint

```diff
 ***REMOVED*** 🧪 Testing framework and plugins
 ***REMOVED*** -------------------------------
-pytest>=7.4.3,<8.0                          ***REMOVED*** Core test runner
+pytest>=7.4.3,<9.0                          ***REMOVED*** Core test runner (allow 8.x for HA compatibility)
 pytest-cov>=4.1.0,<5.0                      ***REMOVED*** Coverage reporting
 pytest-asyncio>=0.21.1,<1.0                 ***REMOVED*** Async test support
-pytest-homeassistant-custom-component>=0.13.0,<1.0  ***REMOVED*** HA integration test harness
-pytest-mock>=3.12.0,<4.0                    ***REMOVED*** Mocking utilities
+***REMOVED*** Moved to tests/requirements.txt to avoid conflicts

 ***REMOVED*** ... formatting tools ...

 ***REMOVED*** ⚙️ Runtime dependencies (mirrors pyproject.toml)
 ***REMOVED*** -------------------------------
 beautifulsoup4>=4.12.0,<5.0                 ***REMOVED*** HTML parsing
 lxml>=4.9.0,<5.0                            ***REMOVED*** XML/HTML parser backend
 requests>=2.31.0,<3.0                       ***REMOVED*** HTTP client
 aiohttp>=3.9.0,<4.0                         ***REMOVED*** Async HTTP client
-homeassistant>=2025.1.0                     ***REMOVED*** Home Assistant core (for integration testing)
+***REMOVED*** Moved to tests/requirements.txt to avoid conflicts
```

***REMOVED******REMOVED******REMOVED*** File: `tests/requirements.txt`

**Change:** Create separate file for HA test dependencies

```txt
***REMOVED*** Test-specific dependencies
***REMOVED*** These are separate from requirements-dev.txt to avoid dependency conflicts
***REMOVED*** Install both: pip install -r requirements-dev.txt -r tests/requirements.txt

***REMOVED*** Home Assistant testing dependencies
homeassistant>=2025.1.0
pytest-homeassistant-custom-component>=0.13.0,<1.0
pytest-mock>=3.12.0,<4.0
```

***REMOVED******REMOVED******REMOVED*** File: `.vscode/settings.json`
...
***REMOVED******REMOVED******REMOVED*** Windows 11 (Git Bash)
...
***REMOVED******REMOVED******REMOVED*** Expected After Fixes
...
***REMOVED******REMOVED*** Recommended Workflow for Contributors

***REMOVED******REMOVED******REMOVED*** Quick Setup (Formatting & Linting Only)
...
***REMOVED******REMOVED******REMOVED*** Full Setup (All Tests)
...
***REMOVED*** Install test dependencies separately
source .venv/bin/activate  ***REMOVED*** or .venv\Scripts\activate on Windows
...
***REMOVED******REMOVED******REMOVED*** Docker Development (Easiest)

```bash
git clone https://github.com/kwschulz/cable_modem_monitor.git
cd cable_modem_monitor
code .
***REMOVED*** Press F1 → "Dev Containers: Reopen in Container"
```
...
***REMOVED******REMOVED******REMOVED*** Without Fixes
...
***REMOVED******REMOVED******REMOVED*** With Fixes
...
***REMOVED******REMOVED*** Priority Order
