...
field, and store the result on the auth config. Three entry points do
it today: ``apply_credential_encoding`` (runtime re-apply), the test
harness runner's ``_detect_form_nonce_encoding``, and the HA config
flow's ``_detect_and_inject_form_nonce_encoding``. The HA helper lives
outside Core and is not imported here (no cross-boundary imports); it
shares ``_analyze_login_form`` with the runner, which these rows pin.
...
from solentlabs.cable_modem_monitor_core.orchestration.factory import apply_credential_encoding
from solentlabs.cable_modem_monitor_core.test_harness.runner import _detect_form_nonce_encoding
...
# ┌───────────────────┬──────────────────────────┬────────────────────────────┬──────────────────────────────┐
# │ start config      │ call args                │ result (encoding, field)   │ description                  │
# ├───────────────────┼──────────────────────────┼────────────────────────────┼──────────────────────────────┤
# │ plain, ""         │ plain                    │ plain, ""                  │ plain stays plain            │
# │ plain, ""         │ b64_packed, arguments    │ b64_packed, arguments      │ packed sets both             │
# │ b64_packed, prev  │ plain                    │ plain, prev                │ plain leaves the old field   │
...
APPLY_FORM_NONCE_CASES = [
    ...
    (FORM_NONCE_PACKED,   "plain",      "",          "plain",           "prev",         "plain_over_packed"),
...
# │ empty body           │ b64_packed, prev  │ b64_packed, prev           │ runner returns untouched     │
# └──────────────────────┴───────────────────┴────────────────────────────┴──────────────────────────────┘
#
# fmt: off
RUNNER_CASES = [
    ...
    (PAGE_EMPTY,      FORM_NONCE_PACKED,  "b64_packed", "prev",      "empty_body"),
]
# fmt: on
...
class TestApplyCredentialEncoding:
    """``apply_credential_encoding`` writes form_nonce config and nothing else."""
    ...
    def test_form_nonce(
        ...
        apply_credential_encoding(config, credential_encoding=encoding, credential_field=field)
    ...
    def test_other_strategies_untouched(self, auth: dict[str, Any] | None) -> None:
        ...
        apply_credential_encoding(config, credential_encoding="b64_packed", credential_field="arguments")
...
class TestRunnerDetection:
    ...
    def test_runner_detect(self, page: str, auth: dict[str, Any], encoding: str, field: str, desc: str) -> None:
        ...
        with RecordingServer("text/html", page) as server:
            _detect_form_nonce_encoding(config, server.base_url)
