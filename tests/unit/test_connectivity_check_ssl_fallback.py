...
# Patch requests in lib/connectivity.py where check_connectivity is defined
CONNECTIVITY_MODULE = "custom_components.cable_modem_monitor.lib.connectivity"
...
class TestConnectivityCheckSSLFallback:
    ...
    def test_connectivity_check_should_fallback_on_ssl_handshake_error(self):
        ...
        with patch(f"{CONNECTIVITY_MODULE}.requests.Session", side_effect=mock_session_factory):
    ...
    def test_connectivity_check_returns_false_for_non_ssl_errors(self):
        ...
        with patch(f"{CONNECTIVITY_MODULE}.requests.Session", return_value=mock_session):
            result = check_connectivity("https://192.168.100.1")
...
class TestConnectivityCheckLogging:
    ...
    def test_successful_connection_logs_at_info_not_warning(self, caplog):
        ...
        with (
            patch(f"{CONNECTIVITY_MODULE}.requests.Session", return_value=mock_session),
    ...
    def test_fallback_attempts_log_at_info_not_warning(self, caplog):
        ...
        with (
            patch(f"{CONNECTIVITY_MODULE}.requests.Session", side_effect=mock_session_factory),
            caplog.at_level(logging.DEBUG),
        ):
            result = check_connectivity("192.168.100.1")
