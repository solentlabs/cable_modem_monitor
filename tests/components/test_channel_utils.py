...
from custom_components.cable_modem_monitor.channel_utils import (
    extract_channel_id,
...
class TestNormalizeChannelType:
    ...
    def test_downstream_qam_from_channel_type(self):
        ...
        assert normalize_channel_type(channel, "downstream") == "qam"
    ...
    def test_downstream_ofdm_from_channel_type(self):
        ...
        assert normalize_channel_type(channel, "downstream") == "ofdm"
    ...
    def test_upstream_atdma_from_channel_type(self):
        ...
        assert normalize_channel_type(channel, "upstream") == "atdma"
    ...
    def test_upstream_ofdma_from_channel_type(self):
        ...
        assert normalize_channel_type(channel, "upstream") == "ofdma"
    ...
    def test_empty_channel(self):
        ...
        assert normalize_channel_type({}, "downstream") == "qam"
        assert normalize_channel_type({}, "upstream") == "atdma"

    def test_case_insensitivity(self):
        """Channel type matching is case-insensitive."""
        assert normalize_channel_type({"channel_type": "OFDM"}, "downstream") == "ofdm"
        assert normalize_channel_type({"channel_type": "Ofdma"}, "upstream") == "ofdma"
...
class TestExtractChannelId:
    ...
    def test_numeric_string(self):
        ...
        assert extract_channel_id({"channel_id": "1"}, 99) == 1
        assert extract_channel_id({"channel_id": "32"}, 99) == 32
    ...
    def test_channel_field_fallback(self):
        ...
        assert extract_channel_id({"channel": "5"}, 99) == 5
        assert extract_channel_id({"channel": 10}, 99) == 10

    # Prefixed channel IDs (G54 parser style)
    def test_ofdm_prefix(self):
        """OFDM-prefixed channel ID from G54 parser."""
        assert extract_channel_id({"channel_id": "OFDM-0"}, 99) == 0
        assert extract_channel_id({"channel_id": "OFDM-1"}, 99) == 1

    def test_ofdma_prefix(self):
        """OFDMA-prefixed channel ID from G54 parser."""
        assert extract_channel_id({"channel_id": "OFDMA-0"}, 99) == 0
        assert extract_channel_id({"channel_id": "OFDMA-1"}, 99) == 1

    def test_other_prefixes(self):
        """Other prefixed formats should also work."""
        assert extract_channel_id({"channel_id": "CH-5"}, 99) == 5
        assert extract_channel_id({"channel_id": "DS-10"}, 99) == 10

    # Default fallback cases
    def test_missing_channel_id(self):
        """Returns default when no channel_id or channel field."""
        assert extract_channel_id({}, 99) == 99
        assert extract_channel_id({"frequency": 765000000}, 42) == 42

    def test_unparseable_string(self):
        """Returns default for unparseable strings."""
        assert extract_channel_id({"channel_id": "invalid"}, 99) == 99
        assert extract_channel_id({"channel_id": ""}, 99) == 99

    def test_none_value(self):
        """Returns default when channel_id is None."""
        assert extract_channel_id({"channel_id": None}, 99) == 99

    # Edge cases
    def test_whitespace_handling(self):
        """Handles whitespace in channel IDs."""
        assert extract_channel_id({"channel_id": " 5 "}, 99) == 5
        assert extract_channel_id({"channel_id": " OFDM-2 "}, 99) == 2


# ┌─────────────────────────────────────────────────────────────────────────────┐
# │ normalize_channels test data                                               │
# ├────────────────┬────────────────────┬───────────────────────────────────────┤
...
class TestNormalizeChannels:
    ...
    def test_downstream_normalization(self, channels, expected_keys, desc):
        ...
        result = normalize_channels(channels, "downstream")
    ...
    def test_upstream_ofdma_channels(self):
        ...
        result = normalize_channels(channels, "upstream")
    ...
    def test_empty_channels(self):
        ...
        assert normalize_channels([], "downstream") == {}
        assert normalize_channels([], "upstream") == {}
