...
def test_cleanup_orphaned_entities(mock_json_load, mock_json_dump, mock_file, mock_entity_registry_data):
    ...
    # Mock the backup function to avoid filesystem interaction
    with patch("custom_components.cable_modem_monitor.utils.entity_cleanup.backup_entity_registry") as mock_backup:
        result = cleanup_orphaned_entities(None)  # Pass None for hass, as it's not used in the sync version

        assert result is True
        mock_backup.assert_called_once()

        # Verify that json.dump was called with the correct data (without the orphaned entity)
...
def test_remove_all_entities(mock_json_load, mock_json_dump, mock_file, mock_entity_registry_data):
    ...
    with patch("custom_components.cable_modem_monitor.utils.entity_cleanup.backup_entity_registry") as mock_backup:
        result = remove_all_entities(None)  # Pass None for hass

        assert result is True
        mock_backup.assert_called_once()

        # Verify that json.dump was called with the correct data (without any cable modem entities)
        args, kwargs = mock_json_dump.call_args
        written_data = args[0]
