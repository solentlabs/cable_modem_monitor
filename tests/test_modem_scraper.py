...
class TestARRISSB6141:
    ...
    def test_arris_sb6141_downstream_parsing(self, arris_sb6141_html):
        ...
        # Should have 8 downstream channels based on fixture
        assert len(downstream) == 8, f"Expected 8 downstream channels, got {len(downstream)}"

        # Check first channel (ID=10)
        ch = downstream[0]
        assert ch['channel'] == 10
        assert ch['frequency'] == 519000000  # Hz
        assert ch['snr'] == 39.0  # dB
        assert ch['power'] == 5.0  # dBmV
    ...
    def test_arris_sb6141_upstream_parsing(self, arris_sb6141_html):
        ...
        # Should have 4 upstream channels based on fixture
        assert len(upstream) == 4, f"Expected 4 upstream channels, got {len(upstream)}"

        # Check first channel (ID=7)
        ch = upstream[0]
        assert ch['channel'] == 7
        assert ch['frequency'] == 30600000  # Hz
        assert ch['power'] == 49.0  # dBmV

        # Check last channel (ID=6)
        ch = upstream[3]
        assert ch['channel'] == 6
    ...
    def test_arris_sb6141_error_stats_merged(self, arris_sb6141_html):
        ...
        # All downstream channels should have corrected/uncorrected values
        for ch in downstream:
