...
def network_setup_html():
    ...
    return load_fixture("technicolor", "xb7", "network_setup.jst")
...
class TestDetection:
    ...
    def test_by_url(self, soup, network_setup_html):
        """Test detection by content patterns via HintMatcher."""
        hint_matcher = HintMatcher.get_instance()
        matches = hint_matcher.match_login_markers(network_setup_html)
        assert any(m.parser_name == "TechnicolorXB7Parser" for m in matches)

    def test_by_content(self, soup, network_setup_html):
        """Test detection by HTML content patterns via HintMatcher."""
        hint_matcher = HintMatcher.get_instance()
        matches = hint_matcher.match_login_markers(network_setup_html)
        assert any(m.parser_name == "TechnicolorXB7Parser" for m in matches)

    def test_rejects_wrong_content(self):
        """Test that wrong content is not detected via HintMatcher."""
        wrong_html = "<html><body>Some random page</body></html>"
        hint_matcher = HintMatcher.get_instance()
        matches = hint_matcher.match_login_markers(wrong_html)
        assert not any(m.parser_name == "TechnicolorXB7Parser" for m in matches)
